...
int32_t draw_text(cairo_t *cairo, int32_t width, int32_t height,
                  const char *str, int32_t x, double scale,
...
static void draw_content(cairo_t *cairo, int32_t width, double scale) {
  ...
  if (lines > 0) {
    int row = 1;
    for (Item *item = page_start; item && row <= lines;
         item = item->right, row++) {
  ...
  if (matches) {
    ...
    for (item = leftmost; item; item = item->right) {
      ...
      if (x < width) {
        ...
        x = draw_text(cairo, width - 20 * scale, row_height, item->text, x,
...
void draw(cairo_t *cairo, int32_t width, int32_t height, double scale) {
  int32_t max_border = (MIN(width, height) - 1) / 2;
  int64_t scaled_border = (int64_t)border_width * scale;
  int32_t border =
      scaled_border < max_border ? scaled_border : MAX(0, max_border);
  ...
  cairo_save(cairo);
  cairo_rectangle(cairo, border, border, width - 2 * border,
                  height - 2 * border);
  cairo_clip(cairo);
...
static int32_t parse_nonnegative(const char *option, const char *value) {
  char *end;
...
static void measure_layout(int32_t *width, int32_t *height) {
  ...
  for (item = items; item; item = item->next) {
    ...
    if (!fstrncmp(text, item->text, len + 1)) {
      appenditem(item, &lexact, &exactend);
               ...
               !fstrncmp(tokens[0], item->text, strlen(tokens[0]))) {
...
void usage(void) {
  ...
  printf(
      "  -mw, --min-width N                minimum floating width in pixels\n");
  printf(
      "  -bw, --border-width N             border width in logical pixels\n");
  printf("  -bc, --border-color COLOR         border color (#RRGGBB or "
         "#RRGGBBAA)\n");
  printf("  -m,  --monitor MONITOR            output index or name on which to "
         "appear\n");

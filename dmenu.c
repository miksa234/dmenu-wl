...
void
keypress (struct dmenu_panel *panel, enum wl_keyboard_key_state state,
    ...
    switch (sym) {
        ...
        case XKB_KEY_Delete:
                ...
                return;
            {
                ...
                memmove (text + cursor, text + next_cursor,
                         len - next_cursor + 1);
                match ();
            }
            break;
...
int32_t
draw_text (cairo_t *cairo, int32_t width, int32_t height, const char *str,
           int32_t x, double scale, uint32_t foreground_color,
           uint32_t background_color, int32_t padding)
{
    ...
    if (x + padding * scale + text_width + 30 * scale > width) {
        ...
        if (background_color) {
            cairo_set_source_u32 (cairo, background_color);
            cairo_rectangle (cairo, x, 0, text_width + 2 * padding * scale,
                             height);
            cairo_fill (cairo);
        }
...
draw_content (cairo_t *cairo, int32_t width, double scale)
{
    ...
    cairo_rectangle (cairo, x, 0, input_end - x, row_height);
    cairo_clip (cairo);
    draw_text (cairo, input_end, row_height, text, x, scale, color_input_fg, 0,
               6);
    ...
    if (matches) {
        ...
        for (item = leftmost; item; item = item->right) {
            ...
            if (x < width) {
                x = draw_text (cairo, width - 20 * scale, row_height,
                               item->text, x, scale, fg_color, bg_color,
                               item_padding);
            } else {
                break;
            }

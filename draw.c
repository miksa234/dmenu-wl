...
int32_t round_to_int(double val) { return (int32_t)(val + 0.5); }
...
static void keyboard_modifiers(void *data, struct wl_keyboard *keyboard,
  ...
  (void)serial;
  struct dmenu_panel *panel = data;
  xkb_state_update_mask(panel->keyboard.xkb_state, mods_depressed, mods_latched,
...
void dmenu_init_panel(struct dmenu_panel *panel, int32_t width, int32_t height,
  ...
  panel->surface.fractional_scale =
      wp_fractional_scale_manager_v1_get_fractional_scale(
          panel->display_info.fractional_scale_manager, panel->surface.surface);
...
void dmenu_close(struct dmenu_panel *dmenu) { dmenu->running = false; }
